...
<script>
    ...
    function rerunCompliance(audit){
    	...
    	UIkit.modal.confirm('<h2>RERUN COMPLIANCE SELECTION?</h2><h3>Are you sure you want to rerun the automated compliance selection? Depending on how many are currently being processed, this process can take up to 10 minutes to complete.</h3><hr class="dashed-hr uk-margin-bottom"><em>WARNING!</em> While this will not affect the days and times you have auditors scheduled for your audit, it will remove any assignments you made for your auditors to inspect specific areas and units.<br /><small>THIS WILL NOT RUN ON AUDITS WITH FINDINGS.</small><hr class="dashed-hr uk-margin-bottom"><p><strong>Why does it take up to 10 minutes?</strong> When a compliance selection is run, it actually performs all the selections based on the criteria for each program supported by the system - 10 times. Because each run uses an element of randomness, we then look at the 10 audit options and pick the one that has the highest amount of program overlap to units. This keeps the audit compliant to the federal rules while also making the most efficient use of your time.</p>').then(function() {
