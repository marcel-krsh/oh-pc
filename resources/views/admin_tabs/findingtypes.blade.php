    ...
    <table class="uk-table uk-table-hover uk-table-striped uk-table-condensed small-table-text">
        ...
        <tbody>
            @foreach($findingtypes as $data)
                <tr>
                    <td><a onclick="dynamicModalLoad('admin/finding_type/create/{{$data->id}}')" class="uk-link-muted"><small >{{$data->name}}: @forEach($data->huds as $hud) {{$hud->name}}<br /> @endForEach</small></a></td>
