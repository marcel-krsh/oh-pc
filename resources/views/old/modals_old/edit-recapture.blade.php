<div id="dynamic-modal-content">
	...
	<div class="uk-container uk-container-center"> <!-- start form container -->
		<div class="uk-grid">
			<div class="uk-width-1-1@m uk-width-1-1@s">
				<H3>Edit Recapture <small>{{$recapture->id}}</small></H3>
				<form id='transactionForm' class="uk-form-horizontal" role="form">
					<div class="uk-form-row">
                        <label class="uk-form-label" for="category">Category</label>
                        <div class="uk-form-controls">
						    <input type="text" id="category" name="category" class="uk-input uk-form-small uk-form-width-small" style="width:160px;" value="{{$recapture->expenseCategory->expense_category_name}}" disabled />
                        </div>
                    </div>
					<div class="uk-form-row">
                        <label class="uk-form-label" for="amount">Amount</label>
                        <div class="uk-form-controls">
						    <input type="text" id="amount" name="amount" class="uk-input uk-form-small uk-form-width-small" style="width:80px;" value="{{money_format('%n', $recapture->amount)}}"/>
                        </div>
                    </div>
                    <div class="uk-form-row">
                        ...
                        <div class="uk-form-controls">
							...
							<textarea id="description" name="description" class="uk-textarea  uk-width-1-1">{{$recapture->description}}</textarea>
			...
			<div id="applicant-info-update">
				<div class="uk-grid uk-margin">
					<div class="uk-width-1-3">
						<a class="uk-button  uk-button-danger uk-width-1-1" onclick="deleteRecapture();"> DELETE</a>
					</div>
					<div class="uk-width-1-3">
	...
	function savetransaction() {
		...
		if(no_alert){
			$.post('recapture/{{$recapture->id}}/save', {
	...
	function deleteRecapture(){
		UIkit.modal.confirm("Are you sure you want to delete this recapture?").then(function() {
	        $.post('{{ URL::route("recapture.delete", [$recapture->id]) }}', {
				'_token' : '{{ csrf_token() }}'
			}, function(data) {
