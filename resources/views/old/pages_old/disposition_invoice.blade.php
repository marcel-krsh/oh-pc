...
<div id="invoicepanel">
	<div class="uk-panel uk-panel-box uk-panel-box-white">
		<div class="uk-panel uk-panel-header uk-hidden@m uk-hidden@l" style="text-align:center;">
			...
			<h6 class="uk-panel-title uk-text-center"><span class="blue uk-text-bold	">OHIO HOUSING FINANCE AGENCY</span><br /><span class="green">NEIGHBORHOOD INITIATIVE PROGRAM</span><br />DISPOSITION INVOICE {{$invoice->id}}</h6>
		</div>
		<div class="uk-panel uk-panel-header uk-visible@m">
			...
			<h6 class="uk-panel-title uk-text-center uk-text-left-small"><span class="blue uk-text-bold	">OHIO HOUSING FINANCE AGENCY</span><br /><span class="green">NEIGHBORHOOD INITIATIVE PROGRAM</span><br />DISPOSITION INVOICE {{$invoice->id}}</h6>
			...
			<div uk-grid>
				<div class="uk-width-1-2@s uk-panel">
					...
					<table class="uk-table property-summary">
				        <thead>
				            <tr>
				                <th class="uk-width-1-5">Disposition</th>
				                <th class="uk-width-1-5">Parcel</th>
				                <th class="uk-width-1-5">Status</th>
				                @if(Auth::user()->isHFADispositionApprover() || Auth::user()->isHFAAdmin())
				                <th class="uk-width-1-5">Release</th>
				                @endif
				                <th class="uk-width-1-5 uk-text-right">Total Amount</th>
				            </tr>
				        </thead>
				        <tbody>
				        @if($invoice->dispositions)
				        	@foreach ($invoice->dispositions as $disposition)
				            <tr>

				                <td><a onclick="window.open('/dispositions/{{$disposition->parcel_id}}/{{$disposition->id}}', '_blank')" class="uk-link-muted" uk-tooltip="OPEN DISPOSITION">{{$disposition->id}}</a> @if($legacy != 1) <span uk-icon="close" class="uk-text-muted" onclick="removeDisposition({{$disposition->id}});" uk-tooltip="REMOVE DISPOSITION FROM INVOICE"></span> @endif </td>
				                <td><a onclick="window.open('/viewparcel/{{$disposition->parcel_id}}', '_blank')" class="uk-link-muted" uk-tooltip="OPEN PARCEL">{{$disposition->parcel['parcel_id']}}</a></td>
				                <td>@if(!is_null($disposition->status->invoice_status_name)) {{$disposition->status->invoice_status_name}} @endif</td>
				                @if(Auth::user()->isHFADispositionApprover() || Auth::user()->isHFAAdmin())
				                <td>
				                	@if($disposition->date_release_requested == null && $disposition->release_date == null)
									<a onclick="requestRelease({{$disposition->id}});" class="uk-link-muted" uk-tooltip="REQUEST RELEASE"><span class="a-envelope-4"></span></a>
									@endif
									@if($disposition->date_release_requested !== null && $disposition->release_date == null)
									<span uk-tooltip="REQUESTED RELEASE ON {{ Carbon\Carbon::parse($disposition->date_release_requested)->format('m/d/Y') }}"><span class="a-envelope-4" class="uk-text-muted"></span></span> <a onclick="released({{$disposition->id}});" class="uk-link-muted" uk-tooltip="RELEASE"><span class="a-circle-checked"></span></a>
									@endif
									@if($disposition->date_release_requested !== null && $disposition->release_date !== null)
									<span uk-tooltip="REQUESTED RELEASE ON {{ Carbon\Carbon::parse($disposition->date_release_requested)->format('m/d/Y') }}, RELEASED ON {{ Carbon\Carbon::parse($disposition->release_date)->format('m/d/Y') }}">{{ Carbon\Carbon::parse($disposition->release_date)->format('m/d/Y') }}</span>
									@endif
				                </td>
								@endif
				                <td class="uk-text-right">{{$disposition->total()}}</td>
...
?>
					...
					<div class="uk-form-row">
						...
						<div class="uk-width-1-1 uk-grid no-print">
							<div class="uk-width-1-3@m uk-width-1-1@s uk-margin-top"><a class="uk-button uk-width-1-1 uk-button-primary" onclick="submitDispositionInvoiceNote({{$invoice->id}})"><span class="a-floppy"></span> SAVE</a></div>
...
@if( ((count($invoice->transactions) || Auth::user()->isHFAAdmin() || Auth::user()->isHFAFiscalAgent()) && $isReadyForPayment === 1) || ($legacy == 1 && Auth::user()->isHFAAdmin() || Auth::user()->isHFAFiscalAgent()))
		...
		<div class="uk-panel">
			<div uk-grid class="approvals">
				...
				<div class="uk-width-1-1 no-print">
					<button style="margin-top:5px" class="uk-button uk-button-success uk-float-right uk-width-1-3@m uk-width-1-1@s" type="button" onclick="dynamicModalLoad('transaction/newFromDispositionInvoice/{{$invoice->id}}')"><span class="a-credit-card-plus"></span> NEW PAYMENT</button>
				</div>
				@else
				<div class="uk-width-1-1 no-print">
...
@if(!$legacy && $invoice->status_id != 1)
										...
										<div class="uk-display-inline" id="list-item-upload-box">
						                    ...
						                    $(function(){
						                        ...
						                        settings    = {
						                            ...
						                            url: '{{ URL::route("disposition_invoice.uploadSignature", $invoice) }}',
						                            multiple: false,
						                            ...
						                            completeAll: function (response) {
					                                    ...
					                                    UIkit.modal.prompt("I uploaded and categorized the document(s) accordingly. Please add your comment for the history record.",'',{stack: true}).then(function(val){
					                                        $.post('{{ URL::route("disposition_invoice.uploadSignatureComments", $invoice) }}', {
		...
		@if($isReadyForPayment && $invoice->status_id == 7)
		<div class="uk-panel uk-panel-divider">
			<div uk-grid>
				<div class="uk-panel uk-width-1-2@m uk-width-1-1@s uk-container-center uk-margin-bottom uk-text-center">
					<button style="margin-top:5px" class="uk-button uk-button-success uk-width-1-1@m" type="button" onclick="sendForPayment();">Send approved invoice to Landbank for payment.</button>
				</div>
			</div>
		</div>
		@endif
...
<div class="uk-panel uk-panel-divider uk-margin-bottom uk-margin-top uk-margin-left uk-margin-right">
	<div uk-grid>
		<div class="uk-panel uk-width-1-2@m uk-width-1-1@s uk-container-center uk-text-center no-print">
			<button class="uk-button uk-button-success uk-width-1-1@s" type="button" onclick="requestRelease();"><span class="a-envelope-4"></span> Request Release For All The Dispositions On This Invoice</button>
		</div>
	</div>
</div>
@endif
@if($display_release_button)
<div class="uk-panel uk-panel-divider uk-margin-bottom uk-margin-top uk-margin-left uk-margin-right">
	<div uk-grid>
		<div class="uk-panel uk-width-1-2@m uk-width-1-1@s uk-container-center uk-text-center no-print">
			<button class="uk-button uk-button-success uk-width-1-1@s" type="button" onclick="released();"><span class="a-checkbox-checked"></span> Release All The Dispositions On This Invoice</button>
		</div>
	</div>
</div>
@endif
...
function submitForApproval(){
	UIkit.modal.confirm("Are you sure you want to start the HFA approval process?").then(function() {
	        $.post('{{ URL::route("disposition_invoice.submitForApproval", [$invoice->id]) }}', {
				'_token' : '{{ csrf_token() }}'
			}, function(data) {
				if(data['message']!=''){
...
function requestRelease(disposition_id = 0){

	if(disposition_id == 0){
		var m = "Are you sure you want to request the release of all the dispositions on this invoice?";
	}else{
		var m = "Are you sure you want to request the release of this disposition?";
	}
	UIkit.modal.confirm(m).then(function() {
        $.post('{{ URL::route("disposition_invoice.requestRelease", [$invoice->id]) }}', {
			'_token' : '{{ csrf_token() }}',
...
function released(disposition_id = 0){
	if(disposition_id == 0){
		var m = "Are you sure you want to release of all the dispositions on this invoice?";
	}else{
		var m = "Are you sure you want to release this disposition?";
	}
	UIkit.modal.confirm(m).then(function() {
        $.post('{{ URL::route("disposition_invoice.released", [$invoice->id]) }}', {
			'_token' : '{{ csrf_token() }}',
			'disposition_id' : disposition_id
		}, function(data) {
			if(data['message']!=''){
...
@if((Auth::user()->isHFAAdmin() || Auth::user()->isHFADispositionApprover()) && $invoice->status->id == 3 && $isApproved == 1)
	function submitInvoice(){
		UIkit.modal.confirm("Are you sure you want to submit the invoice?").then(function() {
	        $.post('{{ URL::route("disposition_invoice.submitInvoice", [$invoice->id]) }}', {
				'_token' : '{{ csrf_token() }}'
			}, function(data) {
				if(data['message']!=''){
					UIkit.modal.alert(data['message']);
...
@endif
	...
	function sendForPayment(){
		UIkit.modal.confirm("Are you sure you want to notify the Landbank Disposition Approvers and send this invoice for payment?").then(function() {
	        $.post('{{ URL::route("disposition_invoice.sendForPayment", [$invoice->id]) }}', {
				'_token' : '{{ csrf_token() }}'
			}, function(data) {
				if(data['message']!=''){
					UIkit.modal.alert(data['message']);
	...
	function approve(approval_type = 12){
...
@if(Auth::user()->isHFAAdmin())
	function addApprover(id,approval_type = 12){
		$.post('{{ URL::route("disposition_invoice.addApprover", [$invoice->id]) }}', {
			'_token' : '{{ csrf_token() }}',
			'approval_type' : approval_type,
			'user_id' : id
		}, function(data) {
...
@endif
	...
	function removeDisposition(id){
		@if($hasApprovals)
		UIkit.modal.alert("If you would like to remove this parcel, please delete all approvals that have been given first.");
		@elseif($legacy == 1)
		UIkit.modal.alert("Because this is a legacy invoice, parcels cannot be removed.");
		@else
		UIkit.modal.confirm("Are you sure you want to remove this disposition from the invoice?").then(function() {
			$.post('{{ URL::route("disposition_invoice.removeDisposition", [$invoice->id]) }}', {
				'disposition' : id,
				'_token' : '{{ csrf_token() }}'
			}, function(data) {
				if(data['message']!=''){
	...
	function decline(approval_type = 12){
	...
	function remove(id,approval_type = 12){
	...
	function submitDispositionInvoiceNote(id) {
		var form = $('#newNoteForm');

		data = form.serialize();
		$.post('{{ URL::route("dispositioninvoicenote.create", $invoice->id) }}', {
