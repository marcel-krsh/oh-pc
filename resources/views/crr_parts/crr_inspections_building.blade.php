...
@if(!is_null($inspections))
	...
	<div uk-grid class="uk-margin-bottom building">
		...
		<div class="uk-width-1-1 crr-blocks" style="page-break-inside: avoid;">
			<h2>{{number_format($inspections->total(), 0)}} @if(count($inspections) > 1 || count($inspections) < 1) Buildings @else Building @endIf @if($dpView) Selected: @else Audited: @endIf </h2><small><i class="a-mobile"></i> : PHYSICAL INSPECTION </small>
