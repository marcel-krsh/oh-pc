...
@endif
	...
	<form id="userForm" action="{{ route('user.edit-phone-of-user', $up->user->id) }}" method="post" role="userForm">
		...
		<div class="uk-grid">
          ...
          <input id="business_phone_number" type="text" class="uk-input uk-width-1-3" name="business_phone_number" placeholder="Format: xxx-xxx-xxxx" value="{{ $up->phone_number_formatted_noext() }}">
          <input id="phone_extension" type="number" class="uk-input uk-width-1-3" name="phone_extension" placeholder="xxxx" value="{{ $up->phone->extension }}">
        </div>
			</div>
		</div>
		<div class="uk-grid">
...
<script type="text/javascript">
	...
	function submitNewPhone() {
		...
		jQuery.ajax({
			url: "{{ URL::route("user.edit-phone-of-user", $up->user->id) }}",
			method: 'post',
			data: {
				user_id: {{ $up->user->id }},
				project_id: {{ $project_id }},
				phone_number_id: {{ $up->phone->id }},
