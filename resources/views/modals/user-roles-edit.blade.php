	  						...
	  						<form id="user-roles-form" method="post" class="uk-width-1-1 uk-margin-bottom">
	  							<div class="uk-grid-small" uk-grid>
	  								<div class="uk-margin uk-grid-small uk-child-width-auto uk-grid">
							            ...
							            <label><input class="uk-radio" type="checkbox" name="roles" value="{{$role->id}}" @if($user->hasRole($role->id)) @php $checked = 1; @endphp checked @endif> {{$role->role_name}}</label>
							            @endforeach
							            <label><input class="uk-radio" type="checkbox" name="roles" value="0" @if($checked) checked @endif> No Access</label>
							            @endif
							        </div>
