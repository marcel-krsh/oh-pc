...
<div class="communication-selector uk-scrollable-box" style="height: 300px">
	<ul class="uk-list document-menu">
		...
		<li class="findings-list-item finding-{{$f->id}}">
			...
			@endphp
			<input {{ $all_findings == $f->id ? 'checked=checked' : '' }} name="findings[]" id="list-finding-id-{{$f->id}}" value="{{$f->id}}" type="checkbox" class="uk-checkbox" onClick="addFinding(this.value,'{{ $f_icon }}Finding-{{ ($f->id) }}')">
