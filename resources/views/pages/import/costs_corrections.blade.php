	...
	<div class="uk-grid">
		<div class="uk-width-1-1">
			<form action="" method="post">
				<input class="uk-input" type="hidden" name="program_id" value="{{$program_id}}">
				<input class="uk-input" type="hidden" name="account_id" value="{{$account_id}}">

				@foreach ($corrections as $r => $row)
					@foreach ($row as $c => $value)
						<input class="uk-input" type="hidden" name="corrections_{{ $r }}_{{ $c }}" value="{{ $value }}">
					@endforeach
				@endforeach
				<div class="uk-article">
					<div class="uk-block uk-block-primary uk-dark">
									...
									<tr>
										...
										@foreach ($ic[$r] as $c => $cell)
											<td class="{{ isset($ie[$r][$c]) ? 'error' : '' }}">
												...
												@else
													<input type="text" name="corrections_{{ $r}}_{{ $cell['db_col_num'] }}" value="{{ $cell['excel_val'] }}" class="uk-input {{ isset($ie[$r][$c]) ? 'uk-form-danger' : 'no-border no-background auto-width' }}" {{ !isset($ie[$r][$c]) ? 'readonly="true"' : '' }}>
												@endif
				...
				<div class="uk-form-row">
					<button type="submit" name="submit" class="uk-margin-top uk-button uk-button-default blue-button uk-button-large uk-dark uk-width-1-1">SUBMIT CORRECTIONS</button>
					<p>
