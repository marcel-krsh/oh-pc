...
<div class="uk-width-1-1 uk-width-2-3@m uk-align-center uk-margin-top">
    ...
    height: 80px;
    font-size: 18px;"><span class="a-circle-from-bottom uk-icon-large uk-align-center uk-margin-top"></span></div><br /><h1 class="uk-panel-title">Let's Import Some Parcels</h1>
			...
			<input type="hidden" name="table" id="table" value="parcels">
				...
				@if(count($programs)>1)
				<select name="program_id" class="uk-select uk-width-1-1 uk-margin-bottom" id="select-program-id">
	 ...
	 @if(count($programs)>1)
	 	<script>
		 	function getTemplate(){
		 		...
		 		if('#program-id').val().length < 1){
					UIkit.modal.alert('<h2>Oops!</h2><p>Please select which program you\'re for which you\'re creating this template.</p><small>I have great grammar don\'t I? ;) </small>');
				} else {
					$('#getTemplate').submit();
				}
}
	 	...
	 	<form method="GET" action="/import_parcels_template" id="getTemplate">
	 		<input type="hidden" name="program-id" value="">
