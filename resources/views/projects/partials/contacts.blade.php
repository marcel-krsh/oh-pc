				...
				<tbody>
					...
					<tr class="{{ !$user->active ? 'uk-text-muted' : '' }}">
						...
						<td>
							...
							@endphp
							<input class="uk-radio" onchange="makeDefaultOrganization({{ $user->organization_details->id }}, {{ $user->id }},  1)" name="organization" id="organization-{{ $user->organization_details->id }}" type="radio" uk-tooltip="" title="MAKE THIS DEFAULT ORGANIZATION FOR REPORT" aria-expanded="false" {{ (($exists_in_uo && $exists_in_uo->default) || (!$default_org && $default_user_id == $user->id)) ? 'checked=checked': '' }}>
							<small>
					...
					<td>
						<small>@if($user->organization_details)
