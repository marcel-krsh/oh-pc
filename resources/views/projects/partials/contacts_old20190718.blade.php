				...
				<tr class="{{ !$user->active ? 'uk-text-muted' : '' }}">
					...
					@endphp
						<td>
							...
							@if($pm_access)
							<span onclick="editUser({{ $user->id }})" data-uk-tooltip title="{{ $pm_access ? 'User has access to report' : 'No access to report' }} <br> {{ $user->role_name ? 'Role: ' . $user->role_name : '' }}"> <i class="{{ $pm_access ? 'a-file-approve' : 'a-file-fail' }}"></i>
							</span>
							@else
							@endif
						</td>
					@else
						...
						<span class="use-hand-cursor" data-uk-tooltip title="{{ $pm_access ? 'Remove access to report' : 'No access to report' }} <br> {{ $user->role_name ? 'Role: ' . $user->role_name : '' }}" onclick="editUser({{ $user->id }})"><i class="{{ $pm_access ? 'a-file-approve' : 'a-file-fail' }}"></i>
						</span>
					</td>
					@endif
				</tr>
