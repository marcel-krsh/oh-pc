	...
	<div class="uk-grid-collapse" uk-grid>
		...
		<div class="document-upload-step-2-selection" style="display: none;">
			...
			<select name="building_unit" id="upload-document-building-unit"class="uk-select filter-drops uk-width-1-1" value=""><option value="reset">BUILDING / UNIT  (OPTIONAL)</option>
				...
				@php
					$building_findings = $allFindings->where('building_id',$uploadBuilding->id);
					$resolved_findings = count(collect($building_findings)->where('auditor_approved_resolution', 1));
					$unresolved_findings = count($building_findings) - $resolved_findings;
					$total_findings = count($building_findings);
				@endphp
				<option value="building-{{ $uploadBuilding->id }}">BIN {{ $uploadBuilding->building_name }} | @if($allFindings !== null) {{ $total_findings }} FINDINGS :: {{ $unresolved_findings }} UNRESOLVED @else NO FINDINGS @endIf
				</option>
				<option disabled>|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||</option>
				@if($uploadBuilding->units != NULL && count($uploadBuilding->units))
				@forEach($uploadBuilding->units->sortBy('unit_name') as $uploadUnit)
					@php
					$unit_findings = $allFindings->where('unit_id',$uploadUnit->id);
					$resolved_findings = count(collect($unit_findings)->where('auditor_approved_resolution', 1));
					$unresolved_findings = count($unit_findings) - $resolved_findings;
					$total_findings = count($unit_findings);
				@endphp
				<option id="document-upload-unit-{{ $uploadUnit->id }}-select" value="unit-{{ $uploadUnit->id }}">UNIT {{ $uploadUnit->unit_name }} | @if($allFindings !== null) {{ $total_findings }} FINDINGS :: {{ $unresolved_findings }} UNRESOLVED  @else NO FINDINGS @endIf
				</option>
				<option disabled>___________________________________________________________________</option>
				@endForEach
