...
<div class=" uk-width-1-1@s uk-width-4-5@m">
	<div uk-grid="">
		<div class="uk-width-1-1@s uk-width-1-6@m" id="audits-dropdown" style="vertical-align: top;">
			<select id="filter-by-audit" class="uk-select filter-drops uk-width-1-1" onchange="filterByAudit();" style="font-size:13px; padding:0px">
				<option value="" selected="">
					FILTER BY AUDIT
				</option>
				@foreach($audits as $audit)
					<option value="{{ $audit->id }}">{{ $audit->id }}</option>
				@endforeach
			</select>
		</div>
		...
		<div class="uk-width-1-1@s uk-width-1-6@m" id="recipient-dropdown" style="vertical-align: top;">
			<select id="filter-by-finding" class="uk-select filter-drops uk-width-1-1" onchange="filterByFinding();" style="font-size:13px; padding:0px">
				<option value="" selected="">
					FILTER BY FINDING
				</option>
				@foreach($findings as $finding)
						<option value="{{ $finding->id }}">{{ $finding->id }}</option>
				@endforeach
			</select>
		</div>
		<div class="uk-width-1-1@s uk-width-1-6@m" id="recipient-dropdown" style="vertical-align: top;">
			<select id="filter-by-owner" class="uk-select filter-drops uk-width-1-1" onchange="filterByOwner();" style="font-size:13px; padding:0px">
				<option value="all" selected="">
					FILTER BY CATEGORY
				</option>
				@foreach($categories as $category)
						<option value="{{ $category->id }}">{{ $category->document_category_name }} : {{ $category->parent->document_category_name }}</option>
				@endforeach
			</select>
		</div>
		<div class="uk-width-1-1@s uk-width-1-6@m" id="recipient-dropdown" style="vertical-align: top;">
	...
	<div class=" uk-width-1-1@s uk-width-1-6@m uk-text-right">
		<div class="uk-align-right uk-label  uk-margin-top ">  {{ count($documents) }} DOCUMENTS </div>
	</div>
</div>
</div>
</div>
