  		...
  		<div uk-grid class="communication-summary @if($message->unseen) communication-unread @endif">
  			...
  			<div class="uk-width-1-5@m uk-width-1-2@s communication-item-tt-to-from uk-margin-small-bottom" onclick="dynamicModalLoad('communication/0/replies/@if($message->parent_id){{ $message->parent_id }} @else{{ $message->id }} @endif')">
  				<div class="communication-item-date-time">
  					...
  					<span>
  						Me, @if(count($message->recipient_details))@foreach($message->recipient_details as $recipient)@if($recipient != $current_user){{ $recipient->full_name() }}@endif{{ !$loop->last ? ', ': '' }}@endforeach @endif
  					</span>
  				</div>
  				@if($message->unseen > 0)
  			...
  			<div class="uk-width-1-5@s communication-type-and-who uk-hidden@m uk-text-right " >
  				<div class="uk-margin-right">
  					...
  					<p style="margin-bottom:0"><a onclick="window.open('/viewparcel/{{ $message->audit_id }}', '_blank')" class="uk-link-muted" uk-tooltip="OPEN PARCEL">{{ $message->audit_id }}</a></p>
  					<p class="uk-visible@m" style="margin-top:0" uk-tooltip="pos:left;title:@if(Auth::user()->isFromEntity(1)) {{ $message->audit->title }} @endif">
