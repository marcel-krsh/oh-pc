    ...
    public function parcelStore(Request $request, $id = null)
    {
        ...
        if (! $id) {
            $parcel = Parcel::create([
                ...
                'owner_id'=>Request::get('program_id'),
                ...
                'target_area_id'=>Request::get('target_area_id'),
                'sale_price'=>Request::get('sale_price'),
                'how_acquired_id'=>Request::get('how_acquired_id'),
                'how_acquired_explanation'=>Request::get('how_acquired_notes'),
                'historic_significance_or_district'=>Request::get('historic_significance'),
                'units'=>Request::get('units'),
                'program_rules_id'=>Request::get('rule_id'),
            ]);
            ...
            if (! is_null(Request::get('lb_validated'))) {
                $lbValidated = Request::get('lb_validated');
                if (Request::get('lb_validated') == 1) {
                    $message = '';
                } else {
            ...
            if ($parcelold->street_address != Request::get('street_address')) {
                $lbValidated = 0;
            }
            if ($parcelold->city != Request::get('city')) {
                $lbValidated = 0;
            }
            if ($parcelold->state_id != Request::get('state_id')) {
                $lbValidated = 0;
            }
            if ($parcelold->zip != Request::get('zip')) {
                $lbValidated = 0;
            }
            if ($parcelold->county_id != Request::get('county_id')) {
                $lbValidated = 0;
            }
            ...
            Parcel::where('id', $id)->update([
                ...
                'hfa_property_status_id'=>Request::get('hfa_property_status_id'),
                'landbank_property_status_id'=>Request::get('landbank_property_status_id'),
            ]);
            $p = Parcel::find($id);
