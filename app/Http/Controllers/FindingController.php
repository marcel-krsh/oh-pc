...
class FindingController extends Controller
{
	...
	public function modalFindings($type, $auditid, $buildingid = null, $unitid = null, $amenityid = null, $toplevel = 0, $refresh_stream = 0, $location_selected = '')
	{
		...
		if ($auditor_access) {
			...
			// }
			if ($refresh_stream) {
				return view('audit_stream.audit_stream', compact('audit', 'checkDoneAddingFindings', 'type', 'findings', 'unit', 'building', 'amenity', 'audits', 'units', 'buildings', 'amenities', 'auditid', 'buildingid', 'unitid', 'amenityid', 'toplevel', 'current_user', 'auditor_access'));
			} else {
				$findings = [];
	...
	public function findingDetails($id, $document_id = null, Request $request)
	{
		...
		$photos = Photo::where('finding_id', $finding->id)
			...
			->get();
		$finding_type = '';
		if (is_null($finding->building_id) && is_null($finding->unit_id)) {
			$finding_type = 'SITE FINDING FOR:';
		} elseif (!is_null($finding->building_id) && is_null($finding->unit_id)) {
			$finding_type = 'BUILDING FINDING FOR BIN:';
		} elseif (is_null($finding->building_id) && !is_null($finding->unit_id)) {
			$finding_type = 'UNIT FINDING FOR UNIT:';
		}
		return view('modals.finding-details', compact('finding', 'communications', 'finding_type', 'photos', 'documents', 'document_id'));
	}
}
