...
class PagesController extends Controller
{
	...
	users roles
	 */
	public function createUserSave(Request $request, $can_create = 0)
	{
		if (Auth::user()->manager_access() || $can_create) {
			$validator = \Validator::make($request->all(), [
			...
			$email_address->email_address_key = null; //$last_record->email_address_key + 1;
			$email_address->save();
				...
				if (count($projects)) {
					...
					if ($allowed) {
						...
						if (null !== $contact && $check > 0) {
							...
							$user = $this->createUserSave($request, 1);
							if ($user && is_int($user)) {
