...
*/
            ...
            if ($dispositionFilter == '%%') { // all parcels
            ...
            if ($advanceFilter != "%%" ||
                ...
                $parcelsNextFilter != "%%" ||
                $parcelsStatusFilter != "%%" ||
                $hfaParcelsStatusFilter != "%%" ||
                Auth::user()->entity_type == "hfa" ||
                Auth::user()->entity_type == "landbank" ||
                $parcelsProgramFilterOperator != "LIKE") {
                if ($parcelsSortBy == 'property_status_options.option_name') {
                ...
                } elseif ($parcelsSortBy == 'hfa_property_status_options.option_name') {
                    $joinTable = 'property_status_options';
                ...
                } elseif ($parcelsSortBy == 'program_name') {
                    $joinTable = 'programs';
                    $joinOnColumn = 'parcels.program_id';
                    $joinToColumn = 'programs.id';
                } elseif ($parcelsSortBy == 'guide_steps.name') {
    ...
    public function userQuickDelete($userId, Request $request)
    {
        ...
        if (is_null($editUser)) {
            ...
            && strlen($editUser->email_token) > 0) {
                ...
                if (($compliances + $documents + $entities + $imports + $notes + $siteVisits > 0 && $editUser->validate_all != 1) || ($compliances + $documents + $imports + $notes + $siteVisits > 0 && $editUser->validate_all == 1)) {
    ...
    public function viewLog($logtype)
    {
        ...
        if ($logtype =='user') {
            $logs = getUserLogs();
        } elseif ($logtype =='entity') {
            $logs = getEntityLogs();
        } elseif ($logtype=='parcel') {
            $logs = getParcelLogs();
        } else {
            $logs = getAllLogs();
        }
    ...
    public function createUser()
    {
        if (Auth::user()->canManageUsers()) {
            ...
            // get entities
            $entities = Entity::where('active', '=', 1)->orderBy('entity_name', 'ASC')->get();

            $tuser = Auth::user();
            $lc = new LogConverter('user', 'createuser');
            $lc->setDesc($tuser->email . ' Started to create a new user ')->setFrom($tuser)->setTo($tuser)->save();
            return view('modals.new-user', compact('lb_roles', 'hfa_roles', 'entities'));
        } else {
            $tuser = Auth::user();
    ...
    public function createUserSave(Request $request)
    {
        if (Auth::user()->canManageUsers()) {
            ...
            $userParams['roles'] = $addedRoles;
            $tuser = Auth::user();
            $lc->setDesc($tuser->email . ' created a user ' . $newUser->email)->setFrom($tuser)->setTo($newUser);
            $lc->save();
            if (strlen($request->password)>0) {
                // store a password update too
                $newUser->update([
    ...
    public function emailsTab(Request $request)
    {
        ...
        if (Session::has('communications-search') && Session::get('communications-search') != '') {
            ...
            if ($current_user->isFromEntity(1)) { // HFA
