...
class AdminToolController extends Controller
{
    ...
    public function entityCreate(Form $form, $id = null)
    {
        ...
        $ownerIds = UserModel::where('active', 1)->orderBy('name')->pluck('id')->toArray();
        $ownerNames = UserModel::where('active', 1)->orderBy('name')->pluck('name')->toArray();
        $selectedOwner=array_fill(0, count($ownerIds), 'false');
        $entity = Entity::where('id', $id)->first();

        if (!$id) {
            $formRows['tag'] = $form->formBuilder("/admin/entity/store", "post", "application/x-www-form-urlencoded", "Create New Entity", "plus-circle");
            $formRows['rows']['ele1'] = $form->text(['Entity Name','entity_name','','Enter new entity','required']);
            $formRows['rows']['ele2'] = $form->text(['Email Address','email_address','','Enter email address','required']);
            $formRows['rows']['ele3'] = $form->text(['Street Address','street_address','','Enter street address','']);
            $formRows['rows']['ele4'] = $form->text(['Street Address 2','street_address_2','','Enter street address 2','']);
            $formRows['rows']['ele5'] = $form->text(['City','city','','Enter city','']);
            $formRows['rows']['ele6'] = $form->selectBox(['Select State','state',$stateIds,$stateNames,$selectedState,'','required']);
            $formRows['rows']['ele7'] = $form->text(['Zip Code','zipcode','','Enter zip code','']);
            $formRows['rows']['ele8'] = $form->text(['Phone','phone','','Enter phone number','']);
            $formRows['rows']['ele9'] = $form->text(['Fax','fax','','Enter fax number','']);
            $formRows['rows']['ele10'] = $form->text(['Web Address','web_address','','Enter web address','']);
            $formRows['rows']['ele11'] = $form->text(['Datatran User','datatran_user','','Enter datatran user','']);
            $formRows['rows']['ele12'] = $form->text(['Datatran Password','datatran_password','','Enter datatran password','']);
            $formRows['rows']['ele13'] = $form->text(['Logo Link','logo_link','','Enter logo address','']);
            $formRows['rows']['ele14'] = $form->selectBox(['Select Owner','owner',$ownerIds,$ownerNames,$selectedOwner,'','']);
            $formRows['rows']['ele15'] = $form->submit(['Create Entity']);
            return view('pages.formtemplate', ['formRows'=>$formRows]);
        } else {
            $owner = User::find($entity->owner_id);
            if ($owner->active == 1) {
                $key = array_search($owner->name, $ownerNames);
                $selectedOwner[$key]= 'true';
            } else {
                $selectedOwner[] = 'true';
                $ownerNames[]= 'Deactivated! ['.$owner->name.']';
                $ownerIds[]=$owner->id;
            }
            $selectedState[intval($entity->state_id)-1]= 'true';
            $formRows['tag'] = $form->formBuilder("/admin/entity/store/".$entity->id, "post", "application/x-www-form-urlencoded", "Edit Entity", "edit");
            $formRows['rows']['ele1'] = $form->text(['Entity Name','entity_name',$entity->entity_name,'','required']);
            $formRows['rows']['ele2'] = $form->text(['Email Address','email_address',$entity->email_address,'','required']);
            $formRows['rows']['ele3'] = $form->text(['Street Address','street_address',$entity->address1,'','']);
            $formRows['rows']['ele4'] = $form->text(['Street Address 2','street_address_2',$entity->address2,'','']);
            $formRows['rows']['ele5'] = $form->text(['City','city',$entity->city,'','']);
            $formRows['rows']['ele6'] = $form->selectBox(['Select State','state',$stateIds,$stateNames,$selectedState,'','required']);
            $formRows['rows']['ele7'] = $form->text(['Zip Code','zipcode',$entity->zip,'','']);
            $formRows['rows']['ele8'] = $form->text(['Phone','phone',$entity->phone,'','']);
            $formRows['rows']['ele9'] = $form->text(['Fax','fax',$entity->fax,'','']);
            $formRows['rows']['ele10'] = $form->text(['Web Address','web_address',$entity->web_address,'','']);
            $formRows['rows']['ele11'] = $form->text(['Datatran User','datatran_user','','','']);
            $formRows['rows']['ele12'] = $form->text(['Datatran Password','datatran_password','','','']);
            $formRows['rows']['ele13'] = $form->text(['Logo Link','logo_link',$entity->logo_link,'','']);
            $formRows['rows']['ele14'] = $form->selectBox(['Select Owner','owner',$ownerIds,$ownerNames,$selectedOwner,'','']);
            $formRows['rows']['ele15'] = $form->submit(['Update Entity']);
            return view('pages.formtemplate', ['formRows'=>$formRows]);
        }
    }
    ...
    {
        $entities = Entity::with(['user','state'])->orderBy('entity_name', 'asc')->get();
        return view('admin_tabs.entity', compact('entities'));
    }
