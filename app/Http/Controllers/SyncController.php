...
class SyncController extends Controller
{
    ...
    public function sync() {
        ...
        if(!is_null($apiConnect)){
            $syncData = $apiConnect->listAudits(1, $modified, 1,'admin@allita.org', 'System Sync Job', 1, 'Server');
            ...
            if($syncData['meta']['totalPageCount'] > 0){
                do{
                    if($syncPage > 1){
                        //Get Next Page
                        $syncData = $apiConnect->listAudits($syncPage, $modified, 1,'admin@allita.org', 'System Sync Job', 1, 'Server');
                        $syncData = json_decode($syncData, true);
                    ...
                    foreach($syncData['data'] as $i => $v)
                        {
                            ...
                            $updateRecord = SyncMonitoring::select('id','allita_id','last_edited','updated_at')->where('utility_allowance_key',$v['attributes']['utilityAllowanceKey'])->first();
                            // convert booleans
