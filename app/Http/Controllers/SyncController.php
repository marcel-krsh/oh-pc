...
use App\Models\SyncProjectProgramStatusType;
...
class SyncController extends Controller
{
    ...
    public function sync() {
        ...
        $lastModifiedDate = SyncProjectProgramStatusType::select(DB::raw("CONCAT(last_edited) as 'last_edited_convert'"),'last_edited','id')->orderBy('last_edited','desc')->first();
        ...
        if(!is_null($apiConnect)){
            $syncData = $apiConnect->listDevelopmentProgramStatusType(1, $modified, 1,'admin@allita.org', 'System Sync Job', 1, 'Server');
            ...
            if($syncData['meta']['totalPageCount'] > 0){
                do{
                    if($syncPage > 1){
                        //Get Next Page
                        $syncData = $apiConnect->listDevelopmentProgramStatusType($syncPage, $modified, 1,'admin@allita.org', 'System Sync Job', 1, 'Server');
                        $syncData = json_decode($syncData, true);
                    ...
                    foreach($syncData['data'] as $i => $v)
                        {
                            ...
                            if(isset($updateRecord->id)) {
                                ...
                                $allitaTableRecord = ProjectProgramStatusType::find($updateRecord->allita_id);
                                /// END NEW CODE PART 1
