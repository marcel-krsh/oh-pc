...
class SyncController extends Controller
{
    ...
    public function sync() {
        ...
        $lastModifiedDate = SyncProjectBuilding::select(DB::raw("CONCAT(last_edited) as 'last_edited_convert'"),'last_edited','id')->orderBy('last_edited','desc')->first();
        ...
        if(!is_null($apiConnect)){
            $syncData = $apiConnect->listProjectBuildings(1, $modified, 1,'admin@allita.org', 'System Sync Job', 1, 'Server');
            ...
            if($syncData['meta']['totalPageCount'] > 0){
                do{
                    if($syncPage > 1){
                        //Get Next Page
                        $syncData = $apiConnect->listProjectBuildings($syncPage, $modified, 1,'admin@allita.org', 'System Sync Job', 1, 'Server');
                        $syncData = json_decode($syncData, true);
                    ...
                    foreach($syncData['data'] as $i => $v)
                        {
                            ...
                            $updateRecord = SyncProjectBuilding::select('id','allita_id','last_edited','updated_at')->where('compliance_contact_key',$v['attributes']['complianceContactKey'])->first();
                            // convert booleans
                            // settype($v['attributes']['floatingUnits'], 'boolean');
                            // settype($v['attributes']['isProjectBuildingHandicapAccessible'], 'boolean');
