...
use App\Models\SyncUnitAmenity;
...
class SyncUnitAmenitiesJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $lastModifiedDate = SyncUnitAmenity::select(DB::raw("CONCAT(last_edited) as 'last_edited_convert'"), 'last_edited', 'id')->orderBy('last_edited', 'desc')->first();
        ...
        if (! is_null($apiConnect)) {
            $syncData = $apiConnect->listUnitAmenities(1, $modified, 1, 'admin@allita.org', 'System Sync Job', 1, 'Server');
            ...
            if ($syncData['meta']['totalPageCount'] > 0) {
                do {
                    if ($syncPage > 1) {
                        //Get Next Page
                        $syncData = $apiConnect->listUnitAmenities($syncPage, $modified, 1, 'admin@allita.org', 'System Sync Job', 1, 'Server');
                        $syncData = json_decode($syncData, true);
                    ...
                    foreach ($syncData['data'] as $i => $v) {
                        ...
                        $updateRecord = SyncUnitAmenity::select('id', 'allita_id', 'last_edited', 'updated_at')->where('unit_amenity_key', $v['attributes']['unitAmenityKey'])->first();
                        // convert booleans
                        // settype($v['attributes']['isActive'], 'boolean');
                        // settype($v['attributes']['isUnitAmenityHandicapAccessible'], 'boolean');
                        ...
                        if (isset($updateRecord->id)) {
                            ...
                            $allitaTableRecord = UnitAmenity::find($updateRecord->allita_id);
                            /// END NEW CODE PART 1
                            ...
                            if ($devcoDateEval > $allitaDateEval) {
                                if (! is_null($allitaTableRecord) && $allitaTableRecord->last_edited <= $updateRecord->updated_at) {
                                    ...
                                    SyncUnitAmenity::where('id', $updateRecord['id'])
                                    ->update([
                                    ...
                                    $UpdateAllitaValues = SyncUnitAmenity::find($updateRecord['id']);
                                ...
                                //dd('inside.');
                                    ...
                                    $allitaTableRecord = UnitAmenity::create([
                                    ...
                                    $syncTableRecord = SyncUnitAmenity::where('id', $updateRecord['id'])
                                    ->update([
                            ...
                            // Create the sync table entry with the allita id
                            $syncTableRecord = SyncUnitAmenity::create([
