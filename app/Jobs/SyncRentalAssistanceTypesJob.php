...
use App\Models\SyncRentalAssistanceType;
...
class SyncRentalAssistanceTypesJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $lastModifiedDate = SyncRentalAssistanceType::select(DB::raw("CONCAT(last_edited) as 'last_edited_convert'"), 'last_edited', 'id')->orderBy('last_edited', 'desc')->first();
        ...
        if (! is_null($apiConnect)) {
            ...
            if ($syncData['meta']['totalPageCount'] > 0) {
                do {
                    if ($syncPage > 1) {
                        //Get Next Page
                        $syncData = $apiConnect->listRentalAssistanceTypes($syncPage, $modified, 1, 'admin@allita.org', 'System Sync Job', 1, 'Server');
                        $syncData = json_decode($syncData, true);
                    ...
                    foreach ($syncData['data'] as $i => $v) {
                        ...
                        if (isset($updateRecord->id)) {
                            ...
                            $allitaTableRecord = RentalAssistanceType::find($updateRecord->allita_id);
                            /// END NEW CODE PART 1
                            ...
                            if ($devcoDateEval > $allitaDateEval) {
                                if (! is_null($allitaTableRecord) && $allitaTableRecord->last_edited <= $updateRecord->updated_at) {
                                    ...
                                    SyncRentalAssistanceType::where('id', $updateRecord['id'])
                                    ->update([
                                    ...
                                    $UpdateAllitaValues = SyncRentalAssistanceType::find($updateRecord['id']);
                                ...
                                //dd('inside.');
                                    ...
                                    $allitaTableRecord = RentalAssistanceType::create([
