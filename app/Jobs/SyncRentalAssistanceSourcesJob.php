...
use App\Models\SyncRentalAssistanceSource;
use App\Models\RentalAssistanceSource;

class SyncRentalAssistanceSourcesJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $lastModifiedDate = SyncRentalAssistanceSource::select(DB::raw("CONCAT(last_edited) as 'last_edited_convert'"), 'last_edited', 'id')->orderBy('last_edited', 'desc')->first();
        ...
        if (!is_null($apiConnect)) {
            $syncData = $apiConnect->listRentalAssistanceSources(1, $modified, 1, 'admin@allita.org', 'System Sync Job', 1, 'Server');
            ...
            if ($syncData['meta']['totalPageCount'] > 0) {
                do {
                    if ($syncPage > 1) {
                        //Get Next Page
                        $syncData = $apiConnect->listRentalAssistanceSources($syncPage, $modified, 1, 'admin@allita.org', 'System Sync Job', 1, 'Server');
                        $syncData = json_decode($syncData, true);
                    ...
                    foreach ($syncData['data'] as $i => $v) {
                            ...
                            $updateRecord = SyncRentalAssistanceSource::select('id', 'allita_id', 'last_edited', 'updated_at')->where('rental_assistance_source_key', $v['attributes']['rentalAssistanceSourceKey'])->first();
                            // convert booleans
                            // settype($v['attributes']['isActive'], 'boolean');
                            // settype($v['attributes']['isRentalAssistanceSourceHandicapAccessible'], 'boolean');
                            //dd($updateRecord,$updateRecord->updated_at);
                        if (isset($updateRecord->id)) {
                            // record exists - get matching table record

                            /// NEW CODE TO UPDATE ALLITA TABLE PART 1
                            $allitaTableRecord = RentalAssistanceSource::find($updateRecord->allita_id);
                            /// END NEW CODE PART 1
                            ...
                            if ($devcoDateEval > $allitaDateEval) {
                                if (!is_null($allitaTableRecord) && $allitaTableRecord->last_edited <= $updateRecord->updated_at) {
                                    ...
                                    SyncRentalAssistanceSource::where('id', $updateRecord['id'])
                                    ->update([
                                    ...
                                    $UpdateAllitaValues = SyncRentalAssistanceSource::find($updateRecord['id']);
                                    ...
                                    $allitaTableRecord = RentalAssistanceSource::create([
                                    ...
                                    $syncTableRecord = SyncRentalAssistanceSource::where('id', $updateRecord['id'])
                                    ->update([
                            ...
                            // Create the sync table entry with the allita id
                            $syncTableRecord = SyncRentalAssistanceSource::create([
