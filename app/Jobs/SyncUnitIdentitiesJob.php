...
use App\Models\SyncUnitIdentity;
use App\Models\UnitIdentity;

class SyncUnitIdentitiesJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $lastModifiedDate = SyncUnitIdentity::select(DB::raw("CONCAT(last_edited) as 'last_edited_convert'"), 'last_edited', 'id')->orderBy('last_edited', 'desc')->first();
        ...
        if (!is_null($apiConnect)) {
            $syncData = $apiConnect->listUnitIdentities(1, $modified, 1, 'admin@allita.org', 'System Sync Job', 1, 'Server');
            ...
            if ($syncData['meta']['totalPageCount'] > 0) {
                do {
                    if ($syncPage > 1) {
                        //Get Next Page
                        $syncData = $apiConnect->listUnitIdentities($syncPage, $modified, 1, 'admin@allita.org', 'System Sync Job', 1, 'Server');
                        $syncData = json_decode($syncData, true);
                        //dd('Page Count is Higher',$syncData);
                    }
                    foreach ($syncData['data'] as $i => $v) {
                            // check if record exists
                            $updateRecord = SyncUnitIdentity::select('id', 'allita_id', 'last_edited', 'updated_at')->where('unit_identity_key', $v['attributes']['unitIdentityKey'])->first();
                            // convert booleans
                            settype($v['attributes']['isActive'], 'boolean');
                            //settype($v['attributes']['unitIdentityDesc'], 'boolean');
                            //dd($updateRecord,$updateRecord->updated_at);
                        if (isset($updateRecord->id)) {
                            // record exists - get matching table record

                            /// NEW CODE TO UPDATE ALLITA TABLE PART 1
                            $allitaTableRecord = UnitIdentity::find($updateRecord->allita_id);
                            /// END NEW CODE PART 1
                            ...
                            if ($devcoDateEval > $allitaDateEval) {
                                if (!is_null($allitaTableRecord) && $allitaTableRecord->last_edited <= $updateRecord->updated_at) {
                                    ...
                                    SyncUnitIdentity::where('id', $updateRecord['id'])
                                    ->update([
                                            
                                            
                                        'unit_identity_description'=>$v['attributes']['unitIdentityDesc'],
                                    ...
                                    $UpdateAllitaValues = SyncUnitIdentity::find($updateRecord['id']);
                                    ...
                                    $allitaTableRecord = UnitIdentity::create([
                                    ...
                                    $syncTableRecord = SyncUnitIdentity::where('id', $updateRecord['id'])
                                    ->update([
                            ...
                            // Create the sync table entry with the allita id
                            $syncTableRecord = SyncUnitIdentity::create([
