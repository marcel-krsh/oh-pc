...
class UnitInspection extends Model
{
    ...
    public function hasSiteInspection()
    {
        if ($this->is_site_visit) {
            return 1;
        } elseif (self::where('program_id', '=', $this->program_id)->where('audit_id', '=', $this->audit_id)->where('unit_id', '=', $this->unit_id)->where('is_site_visit', '=', 1)->count()) {
            return 1;
        }
    ...
    public function hasFileInspection()
    {
        if ($this->is_file_audit) {
            return 1;
        } elseif (self::where('program_id', '=', $this->program_id)->where('audit_id', '=', $this->audit_id)->where('unit_id', '=', $this->unit_id)->where('is_file_audit', '=', 1)->count()) {
            return 1;
        }
    ...
    public function swap_remove(Audit $audit)
    {
        ...
        $check_if_not_last_one = self::where('audit_id', '=', $audit->id)
