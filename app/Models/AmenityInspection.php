...
class AmenityInspection extends Model
{
    ...
    {
        return $this->hasMany(\App\Models\Finding::class, 'amenity_id');
    }
    ...
    {
        $total = self::where('amenity_id', $this->amenity_id)->where('unit_id', $this->unit_id)->where('audit_id', $this->audit_id)->count();
        if ($total > 1) {
            return true;
        } else {
            return false;
    ...
    {
        $total = self::where('amenity_id', $this->amenity_id)->where('building_id', $this->building_id)->where('audit_id', $this->audit_id)->count();
        if ($total > 1) {
            return true;
        } else {
            return false;
    ...
    {
        $total = self::where('amenity_id', $this->amenity_id)->where('project_id', $this->project_id)->where('audit_id', $this->audit_id)->count();
        if ($total > 1) {
            return true;
        } else {
            return false;
